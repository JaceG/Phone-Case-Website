...
Context document for Claude Code. Captures decisions made during planning so
...
- **Kit: Cinematic Websites with AI** — 85+ prompts (site/image/copy) plus
  ...
  Targets v0/Lovable/Bolt/Cursor/Claude Code.
...
placeholders. Dev server config for the Claude browser lives in
`.claude/launch.json` (`dev`).
...
**Device family switch, crossfade, model slider (2026-09-11):** Jace's
direction: function before form; every make/model uses the one iPhone
shell render until real blanks exist. Built: `DeviceSwitch` (iPhone /
Android thumbnails of the current design), `ModelSlider` (rail with a stop
per model in the chosen family; click, drag, arrow keys; snaps), and
`CrossfadeImage` (dissolves between renders when the model changes; every
model shares camera and light so it reads as the case changing shape).
Plumbing for real per-model renders is in place: `products.modelRenders[]`
(phoneModel + hero/threeQuarter/flat/turntable overrides, any empty slot
falls back to the shared `renders`), `rendersFor(design, model)` in
`src/components/store/catalog.ts`, and `RENDERS_MODEL=<slug> pnpm
renders:import` writes into it. Seed now has four Android models (Galaxy
S25 Ultra, Galaxy S25, Pixel 9 Pro active; Pixel 9 coming soon) → 18
variants. Family is derived from `brand` (`apple` → iPhone, else Android).
Deferred by choice: true 3D (R3F + GLB) — crossfade first, 3D if frames
feel flat. Reseeding resets renders to checker placeholders; run
`pnpm renders:import` after every seed.

**Editorial redesign (2026-09-11):** Jace rejected the dark cinematic look
in favour of the Figma reference "Daily Hero 35 — Arkkhe" (file
E1xCViV2T1hHpok5iycz2Q, node 47001:351, a 1440×810 light editorial hero
with a watch). That file's cover page is a flattened bitmap; the prototype
frame has real layers and was read via the Figma MCP. Rebuilt from it:
- Palette: silver ground `#bdc0c9` with CSS-only studio light, shadow falls
  and hairline columns (`.store-ground`), ink `#2a2a2a`, copper `#c08f6d`,
  card `#e8eff0`. Tokens in `src/components/store/store.css`; the whole
  storefront (desktop and mobile) now uses them. The reference's stock
  background photo is NOT used (placeholder discipline).
- Type: Outfit (body), Ibarra Real Nova (serif accent), Michroma standing
  in for the commercial "Adieu" display face. Loaded via next/font in the
  root layout; font vars are also declared in store.css because Tailwind
  prunes unused @theme vars and store.css is outside its pipeline.
- Desktop hero `src/components/store/desktop/EditorialHero.tsx` is a fixed
  1440×810 stage scaled to fit the viewport (uniform scale, like the Figma
  canvas) with every element at its Figma pixel position. Mapping: headline
  = design title; second line = collection's first word; small serif =
  tagline; copper serif = device family (changes with the switch); rule +
  paragraph = story excerpt; "Know more" block = Add to cart; "Watch demo"
  = "Turn it over" (scrolls to the turntable); big product = hero render;
  the three staggered right-edge thumbnails = iPhone / Android / All
  designs; bottom-left card = model picker (rotated render, model name,
  `ModelSlider`, vertical index numerals).
- Icons (search, play circle, orange rule, menu) are the design's exported
  SVGs in `public/store/icons/`. Nav: Collection/Designs open the catalog
  overlay, Process → #details, About → /contact; search glyph = catalog.
- Renders are now rendered with a transparent film (`render.py` sets
  `film_transparent` + RGBA) so they composite onto the light ground.
- The sticky buy bar slides in only after the hero scrolls away (the hero
  has its own CTA and picker).
- The hero product is `SpinningRender`: it plays a pre-rendered frame loop
  on a canvas (pauses on hover), so the case moves without WebGL. Jace's
  direction (2026-09-11): the reference watch "wobbles" but a two-axis
  wobble made him motion-sick; he wants ONE smooth motion "like the Meta
  logo". That is constant-speed rotation about a single fixed axis tilted
  toward the camera (a slowly turning coin). `render.py --tumble 96`
...
- Gotcha when previewing: the browser pane's Mobile viewport preset also
  ...
  `?device=desktop`) or the mobile tree keeps showing in a wide pane.
Lower sections and the mobile tree were retinted to the palette but not
redesigned; they are the next candidates for a from-scratch pass.
